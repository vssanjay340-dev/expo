...
export function PagerView(props: PagerViewProps) {
  ...
  const emitPageScrollStateIfChanged = (state: 'idle' | 'dragging' | 'settling') => {
    ...
    } as PageScrollStateChangedEvent);
  };
  ...
  useImperativeHandle(
    ...
    () => ({
      setPage: (page: number) => {
        // Compose's `animateScrollToPage` throws on out-of-range indices.
        // Match upstream's silent-clamp behavior by suppressing rejections.
        pagerRef.current?.animateScrollToPage(page).catch(() => {});
      },
      setPageWithoutAnimation: (page: number) => {
        pagerRef.current?.scrollToPage(page).catch(() => {});
      },
  ...
  // Auto-wrap each page in `<RNHostView>` so consumers can pass plain RN
  // children — matching upstream `react-native-pager-view`'s contract.
  // Compose's pager slot can't render RN views directly without a host.
  ...
  return (
    <Host style={style ?? { flex: 1 }}>
      <HorizontalPager
        ...
        onPageScroll={onPageScroll ? buildOnPageScrollHandler(onPageScroll) : undefined}
          ...
          onPageScrollStateChanged
            ? (kind) => {
                if (kind === 'start') {
                  ...
                  emitPageScrollStateIfChanged(
                    isScrollInProgressRef.current ? 'settling' : 'idle'
                  );
                }
              }
            : undefined
...
function buildOnPageScrollHandler(
  ...
  // sharing a single closure between the two branches isn't possible.
  if (worklets?.isWorkletFunction?.(userOnPageScroll)) {
    return (currentPage, currentPageOffsetFraction) => {
      'worklet';
      userOnPageScroll({
        nativeEvent: composePageToPageScroll(currentPage, currentPageOffsetFraction),
 ...
 */
function borderRadiusShape(
  style: PagerViewProps['style'],
  rtl: boolean
): BuiltinShape | undefined {
