...
export type PagerViewProps = ViewProps & {
     ...
     *
     * **Platform requirements**: iOS 18+ (built on `onScrollGeometryChange`).
     * On earlier iOS versions the prop attaches but the callback never fires —
     * a dev-mode warning is logged so the silent degradation is visible.
     * Android has no minimum version.
     */
     ...
     * and `settling`.
     *
     * **Platform requirements**: iOS 18+ (built on `onScrollPhaseChange`).
     * On earlier iOS versions the prop attaches but the callback never fires —
     * a dev-mode warning is logged so the silent degradation is visible.
     * Android has no minimum version, and synthesizes the state from
     * Compose's drag interactions plus `isScrollInProgress`.
     */
